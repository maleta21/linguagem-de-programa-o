...
function exe2(){
    ...
    alert(`Múltiplos de 2 ${mult2} Múltiplos de 3 ${mult3} e Múltiplos de 2 e 3 ${mult2e3}`)
}
function exe3(){
    ...
    do {
        codigo = prompt(`Informe código do produto para compra`)
...
function exe6(){
    let nomes = [], vendas = [], percentuais = [], comissao =[]
    // entrada de dados
    for(let i=0;i<5;i++){
    ...
    let menor = comissao[0] // primeiro é menor
    for(let i=0;i<5;i++){
        if (comissao[i] > maior){
            maior = comissao[i]
        }
    ...
    let iMenor = comissao.indexOf(menor) // posição do maior na comissão
...
function exe8(){
    ...
    for(let i=0;i<7;i++){
        ...
        medias.push(Number(prompt(`Informe a media do aluno ${nomes[i]}`)))
    }
